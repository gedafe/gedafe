...
gpw3f-sql - Database Structure for Gpw3f

=head1 SYNOPSIS

Gpw3f is a generic web front-end to PostgreSQL. Generic means in this
context that Gpw3f does not know anything about the structure or
contents of the database it is presenting.

The idea is that Gpw3f looks at a database and presents it on the web.
Because Gpw3f is no AI it expects the database structure and naming to
follow certain rules.

It is also important to note that Gpw3f does not impose any
...
It may be empty if no special information has to be provided.
 ...
 CREATE TABLE meta_fields ( 
    ...
    -- Field is a reference to table X
    meta_fields_reference TEXT,
...
The B<meta_fields_reference> entry must either be B<NULL> or the name
of an other table which gets referenced from this field (Note:
actually we should be able to pull this information from PostgreSQL,
but we have no been able to do so thus far.)
...
Copyright (c) 2000 ISG / DELEK / ETH Zurich, All rights reserved.
