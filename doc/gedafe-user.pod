...
=head2 Exporting Data

If you replace in the URL the C<action> parameter from C<list> or C<listrep> to
C<export>, Gedafe will generate a "Tab-Separated-Values" (tsv) file, that you
can for example in Excel. You can use the C<EXPORT_URL> variable in the HTML
templates to reference the C<export> version of the current view including the
current search.

=head1 TROUBLESHOOTING

=head2 Authentication always fails

Gedafe only works with 'password' or 'crypt' authentication in PostgreSQL. You
have to configure that in the pg_hba.conf file. For example:
...
F<gedafe-sql.pod>, F<gedafe-templates.txt>, F<Text::CPPTemplate>

=head1 COPYRIGHT

Copyright (c) 2000,2001 ETH Zurich, All rights reserved.
...
Fritz Zaucker E<lt>zaucker@ee.ethz.chE<gt>
