...
If you get an empty form when selecting 'Edit' for a record, it could mean that
you didn't put in the first column of the presentation (list) view or the table
(if there isn't a presentation view) the id of the record. Gedafe needs to know
which the id of the record to edit and it does so by using the first column as
