...
Note that this is rule currently assumed by Gedafe, so you must conform to it,
but it will be relaxed in the future (it will remain just a "good practice" :-))
...
Gedafe reads the access rights of the user to determine if a table should be
shown (SELECT must be granted), if a record can be added (INSERT must be
granted) and if a record can be edited/deleted (UPDATE must be granted). Be
careful about access rights in the database: it will determine effectively what
users can and can't do.  You can use groups to manage more easily the access
rights.

=head1 COPYRIGHT

Copyright (c) 2000,2001 ISG / D-ELEK / ETH Zurich, All rights reserved.
