...
Options: ['area','readonly','isearch','date(from=2000,to=2005)']
...
When the resultset is to large the user has the option to narrow down the
...
dates. The I<from> and I<to> fields specify the the range for the year
list.
...
the data contains only dots and digits, in that case it defaults to
"RIGHT" NOWRAP.
