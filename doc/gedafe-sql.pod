...
B<COMMENT> command. If a table has no COMMENT on it, it will be hidden.
...
You can defined a separate description to the list view by setting a comment
on it. If you don't, the table's comment will be taken. For each field in the
view that has the same name of an another already commented field, that
comment will be taken if not defined.
