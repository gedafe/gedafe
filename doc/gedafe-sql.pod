...
The reason we are requiering unique nonspeaking ids is twofold. First it is
good design practice to use nonspeaking keys and second, we need some way to
uniquely identified each database record. In any event, if you need speaking
keys, just use a *_hid on top of the *_id, and you are all set.
