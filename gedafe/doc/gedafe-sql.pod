...
will ofcource be subject to your privileges regarding reading from the
other table.
...
Fixme: This wodget should switch automatically to a combo box if
...
is for you. It presents a file-upload widget in the browser and stores the
...
which allows you to specify a subdirectory below I<file2fs_dir> which is to
receive the file in question.

 file2fs(uploadpath=budgets)

Gedafe will make a filename for the uploaded file up and save it in the
database. The stored filename will be relative to I<file2fs_dir> so that files
will still be found even when you modify the I<uploadpath>.
