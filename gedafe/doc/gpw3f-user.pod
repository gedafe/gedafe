...
gpw3f-user - Generic PostgreSQL Web Frontend (GPW3F) User-Manual

=head1 DESCRIPTION

GPW3F (Generic PostgreSQL Web Frontend) is a web-based database frontend
...
This is made by using the high sofistication of PostgreSQL that permits to
...
a record though a form, then it works for every table in the database. GPW3F
...
GPW3F has the following "special" features:
...
The structure of the GPW3F distribution is as follows:

 .
 |-- bin              binaries (gpw3fd daemon)
 |-- doc              documentation
 |-- lib              
 |   `-- perl
 |       `-- GPW3F    gpw3f main source
 `-- example
...
lib/perl of the GPW3F distribution is found by perl and start the gpw3fd
daemon (you can use the script gpw3fd-ctl to start it with SYSV-init).
 
=head2 The Application Script

GPW3F is actually a library. The application just calls the 'Start' function
from that library to start it up. This is the script that should be called
from the web. It should look as follows:
  
 #!/usr/sepp/bin/speedy -w
 
 use lib "/usr/local/gpw3f/lib/perl";
 ...
 Start(
 	...
 	list_rows      => 15,
 	admin_user     => 'admin',
 	templates      => '/usr/local/gpw3f/templates/demo',
 	documentation_url => 'http://mysite.com/demo-docs',
 );

GPW3F gathers information about the database structure at the first use. This
can take much time, so it is heavily suggested that you use a persistant perl
...
persistant perl is used, the cached data of the applications will go in the
...
application by specifying gpw3f-configuration variables. The following
...
DBI datasource string specifying the database.
...
URL passed to the html templates where the documention of the application is
stored.

=back

=head2 The gpw3fd Daemon

GPW3F uses an external process to manage session data called gpw3fd. This
daemon must be running to make GPW3F work. You can start it in the init phase
using the bin/gpw3fd-ctl script.

=head2 The Database

F<gpw3f-sql.pod> describes how the database should be setup to work with GPW3F.

=head1 USAGE

=head2 Authentication

Authentication is done with the help of gpw3fd. This daemon stores
user/password pairs using a random-generated "ticket", which is stored in a
cookie on the client side. To make these tickets more secure gpw3fd manages an
expiration on these tickets. Every time that ticket is used, it's expiration
is prolongued by a certain amount (configured in the script) of seconds. The
...
The navigation and general use of GPW3F should be straightforward. At the
beginning, you are presented with the "Entry" page that contains links to every
table to edit and every available report.

For forms, the guiding principle while designing GPW3F was 'POST is evil, use
...
included in the GPW3F distribution. The templates are taken from a directory
specified in the startup script with the 'templates' parameter.

The basic idea is that GPW3F places small "elements" of the page currently
...
F<gpw3f-templates.txt> for a description of what elements are used with what
variables.
...
A difficulty that we encountered while developing GPW3F was the caching of
...
released) into GPW3F to allow generating with the HTML interface real reports
in HTML and parhaps also directly in PDF.
...
F<gpw3f-sql.pod>, F<gpw3f-templates.txt>
