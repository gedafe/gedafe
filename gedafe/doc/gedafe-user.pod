...
defines the current schema, i.e. the schema we want to work in.
You do not see tables in other schemas. From Postgres Version
7.2 on this value defaults to "public" if you do not set it.

=item "schema_search_path"

specifies a search path i.e. defines the schemas
where we want to find tables in. This value defaults 
to the value of schema (above) if you do not set it.
...
It is possible to use schema support if Postgres is Version 7.3 or 
newer. This is done with 2 optional config parameters for Start.pm. 
(It has to be verified if Postgres versions < 7.3 run with this
patch)
...
defines the current schema, i.e. the schema we want to work in. 
You do not see tables in other schemas. 
...
specifies a search path i.e. defines the schemas
where we want to find tables in. 

When setting both options, you only get links for tables in "your" 
schema BUT you can access all tables within the search path by their 
names as if they were in the current schema.

This allows you to reorder your tables in different schemas
without changing views and select statements.
Be careful with the syntax: Schemas must be included in
single quotes and separated by commas!

CAVEAT: This feature is quite experimental. Never put 2 tables with
the same name in two schemas (inside your search path), as the results
can be hard to predict. Also always put I<'public'> in your schema search
path. Put the tables I<meta_fields> and I<eta_tables> in the I<'public'> 
schema. 
