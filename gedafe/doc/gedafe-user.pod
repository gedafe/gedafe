...
when the logged in user matches this username. This tag can, for
example, be used to change the background color.
...
Ports to other databases such as Ingres or Oracle.

=item *

We will probably completely rework and improve the templating system,
since it is very primitive right now. A possibility is the usage of
Template::Toolkit.

=item *

We will integrate PearlReports (by Tobias Oetiker
<oetiker@ee.ethz.ch>, not yet released) into Gedafe to allow more
powerful report generation in HTML and perhaps also in PDF directly
from the HTML interface.

=item *

Internal code cleanup and possibly code rewrite.
